...
class LeavesKMeans():
	def leafSelection(self, inImages, outPath):
		...
		for i in range(len(inImages)):
			...
			erd = cv2.erode(image, kernel, iterations = 2)
			# K-means
			Z = image.reshape((-1,3))
			Z = np.float32(Z)
			...
			# Watershed
			markers = cv2.watershed(res2,markers)
			...
			cv2.imwrite(outPath+pathsAndNames[1]+'_{}_kmeans.jpg'.format(i), image) # Сохраняем файл
			outImages.append(outPath+pathsAndNames[1]+'_{}_kmeans.jpg'.format(i))
	...
	def damageSelection(self, inImages, outPath, inMask):
		...
		for i in range(len(inImages)):
			...
			pathsAndNames = inImages[i].rsplit('\\', 1) # Список путей к папке и имен для всех изображений
			pathsAndNames[1] = os.path.splitext(pathsAndNames[1])[0] # Убираем расширение файла из имени
			cv2.imwrite(outPath+pathsAndNames[1]+'_{}_kmeans.jpg'.format(i), res2) # Сохраняем файл
			outImages.append(outPath+pathsAndNames[1]+'_{}_kmeans.jpg'.format(i))
			# imageio.mimsave(outPath+pathsAndNames[1]+'_{}_kmeans.gif'.format(i), [image, mask, res2], duration=0.8) # Сохраняем gif
		return outImages, np.asarray(pix_white)
